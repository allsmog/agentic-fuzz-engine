...
def _docker_wrapper_image_ok(
    ...
    *,
    ...
    try:
        ...
        repo_root = _development_repo_root(env)
        if repo_root is None:
            return True
        resolved.relative_to((repo_root / "tools" / "bin").resolve())
...
def _tool_search_dirs(env: Mapping[str, str]) -> tuple[Path, ...]:
    ...
    checkout_tools = (repo_root / "tools" / "bin",) if repo_root is not None else ()
    return (
        *custom_dirs,
        *checkout_tools,
...
def _development_repo_root(env: Mapping[str, str]) -> Path | None:
    """Use checkout-only helper tools only when a checkout is explicit.

    Released plugins intentionally do not bundle those helpers, so installed
    copies report the corresponding requirement as unavailable instead of
    treating a nearby cache directory as a source checkout.
    """
    explicit = env.get("AGENTIC_FUZZ_ENGINE_ROOT")
    if explicit and (Path(explicit).expanduser() / "tools" / "bin").is_dir():
        return Path(explicit).expanduser().resolve()
    if env.get("AGENTIC_FUZZ_PLUGIN_ROOT"):
        return None
    candidate = Path(__file__).resolve().parents[2]
    return candidate if (candidate / "tools" / "bin").is_dir() else None
