...
with Atlantis DirectedTaskScheduler semantics — priority ordering with agent
preemption, a per-task round budget that rotates exhausted tasks to the tail
with decayed priority, retirement when the sink is finally reached, and drops
when the sink vanishes from the inventory.
