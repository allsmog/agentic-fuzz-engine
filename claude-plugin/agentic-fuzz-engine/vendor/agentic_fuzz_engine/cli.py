...
def main(argv: list[str] | None = None) -> int:
    ...
    fork_scan_parser = subcommands.add_parser("fork-scan")
    fork_scan_parser.add_argument("--source-root", default=None)
    fork_scan_parser.add_argument("--out", default=None)
    fork_scan_parser.add_argument("--vendor-marker", action="append", dest="vendor_markers", default=None)
    fork_scan_parser.add_argument("--manifest-glob", action="append", dest="manifest_globs", default=None)
    fork_scan_parser.add_argument("--consumer-root", default=None)
    fork_scan_parser.add_argument("--dpkg-info-dir", default=None)
    fork_scan_parser.add_argument("--repository-alias", default=None)
    fork_scan_parser.add_argument("--workspace-root", default=None)
    entry_scan_parser = subcommands.add_parser("entry-scan")
    entry_scan_parser.add_argument("--source-root", default=None)
    entry_scan_parser.add_argument("--out", default=None)
    entry_scan_parser.add_argument("--lib-prefix", action="append", dest="lib_prefixes", default=None)
    entry_scan_parser.add_argument(
        "--service-base-suffix", action="append", dest="service_base_suffixes", default=None
    )
    entry_scan_parser.add_argument("--max-files", type=int, default=20_000)
    entry_scan_parser.add_argument("--workspace-root", default=None)
    ...
    differential_parser = subcommands.add_parser("differential-run")
    differential_parser.add_argument("target")
    differential_parser.add_argument(
        "--command-json", action="append", dest="command_json", default=[],
        help="repeat for each implementation; each value must be a JSON argv array",
    )
    differential_parser.add_argument("--label", action="append", dest="labels", default=None)
    differential_parser.add_argument("--corpus-dir", default=None)
    differential_parser.add_argument("--compare", choices=("behavior", "output"), default="behavior")
    differential_parser.add_argument("--auto", action="store_true")
    differential_parser.add_argument("--build-timeout-seconds", type=float, default=600.0)
    differential_parser.add_argument("--max-inputs", type=int, default=2_000)
    differential_parser.add_argument("--per-input-timeout", type=float, default=10.0)
    differential_parser.add_argument("--max-seconds", type=float, default=900.0)
    differential_parser.add_argument("--top", type=int, default=50)
    differential_parser.add_argument("--accept-exit", action="append", dest="accept_exit_codes", type=int, default=None)
    differential_parser.add_argument("--reject-exit", action="append", dest="reject_exit_codes", type=int, default=None)
    differential_parser.add_argument("--env", action="append", dest="declared_env", default=None, metavar="KEY=VALUE")
    differential_parser.add_argument("--workspace-root", default=None)
    sanitizer_build_parser = subcommands.add_parser("sanitizer-build")
    sanitizer_build_parser.add_argument("target")
    sanitizer_build_parser.add_argument("sanitizer", choices=("msan", "tsan"))
    sanitizer_build_parser.add_argument("--timeout-seconds", type=float, default=1_200.0)
    sanitizer_build_parser.add_argument("--env", action="append", dest="declared_env", default=None, metavar="KEY=VALUE")
    sanitizer_build_parser.add_argument("--workspace-root", default=None)
    sanitizer_sweep_parser = subcommands.add_parser("sanitizer-sweep")
    sanitizer_sweep_parser.add_argument("target")
    sanitizer_sweep_parser.add_argument("sanitizer", choices=("msan", "tsan"))
    sanitizer_sweep_parser.add_argument("--corpus-dir", default=None)
    sanitizer_sweep_parser.add_argument("--binary", default=None)
    sanitizer_sweep_parser.add_argument("--max-inputs", type=int, default=2_000)
    sanitizer_sweep_parser.add_argument("--per-input-timeout", type=float, default=15.0)
    sanitizer_sweep_parser.add_argument("--max-seconds", type=float, default=900.0)
    sanitizer_sweep_parser.add_argument("--runs-per-input", type=int, default=1)
    sanitizer_sweep_parser.add_argument("--top", type=int, default=50)
    sanitizer_sweep_parser.add_argument("--env", action="append", dest="declared_env", default=None, metavar="KEY=VALUE")
    sanitizer_sweep_parser.add_argument("--workspace-root", default=None)
    ...
    scoring_parser = subcommands.add_parser("candidate-scoring")
    scoring_parser.add_argument("action", choices=("report", "record", "calibrate"))
    scoring_parser.add_argument("--target", default=None)
    scoring_parser.add_argument("--hypothesis-id", default=None)
    scoring_parser.add_argument("--votes-json", default=None, help="JSON array of exact lens judgments")
    scoring_parser.add_argument("--stream", default="default")
    scoring_parser.add_argument("--rationale", default=None)
    scoring_parser.add_argument("--revision", type=int, default=1)
    scoring_parser.add_argument("--labels-json", default=None, help="JSON array of explicit calibration labels")
    scoring_parser.add_argument("--workspace-root", default=None)
    campaign_db_parser = subcommands.add_parser("campaign-db")
    campaign_db_parser.add_argument("action", choices=("sync", "status", "report"))
    campaign_db_parser.add_argument("--report", default=None)
    campaign_db_parser.add_argument("--target", default=None)
    campaign_db_parser.add_argument("--limit", type=int, default=200)
    campaign_db_parser.add_argument("--workspace-root", default=None)
    schedule_parser = subcommands.add_parser("schedule-plan")
    schedule_parser.add_argument("action", choices=("sync", "list"))
    schedule_parser.add_argument("--now", type=float, default=None)
    schedule_parser.add_argument("--workspace-root", default=None)
    context_parser = subcommands.add_parser("campaign-context")
    context_parser.add_argument("action", choices=("sync", "show"))
    context_parser.add_argument("--target", default=None)
    context_parser.add_argument("--workspace-root", default=None)
    ...
    round_run_parser.add_argument("--fuzz-seconds", type=float, default=None)
    round_run_parser.add_argument("--rss-limit-mb", type=int, default=None)
    round_run_parser.add_argument("--sync-max-inputs", type=int, default=None)
    ...
    round_run_parser.add_argument("--klee-every", type=int, default=None)
    ...
    round_run_parser.add_argument("--min-free-gb", type=float, default=None)
    ...
    if args.command in {"fidelity-list-fixtures", "fidelity-list-fixtures"}:
    ...
    elif args.command == "fork-scan":
        payload = engine.call_tool(
            "fork_scan",
            {
                "source_root": args.source_root,
                "out": args.out,
                "vendor_markers": args.vendor_markers,
    ...
    elif args.command == "entry-scan":
        payload = engine.call_tool(
            "entry_scan",
            {
                "source_root": args.source_root,
                "out": args.out,
                "lib_prefixes": args.lib_prefixes,
                "service_base_suffixes": args.service_base_suffixes,
                "max_files": args.max_files,
                "workspace_root": args.workspace_root,
            },
        )
        ...
        payload = engine.call_tool(
            ...
            {
                "target": args.target,
                "commands": [_load_json_argv(item, key="--command-json") for item in args.command_json],
                "labels": args.labels,
                "corpus_dir": args.corpus_dir,
                ...
                "max_seconds": args.max_seconds,
                "top": args.top,
                "accept_exit_codes": args.accept_exit_codes or [0],
                "reject_exit_codes": args.reject_exit_codes or [1],
                "declared_env": _parse_env_pairs(args.declared_env),
                "workspace_root": args.workspace_root,
            },
        )
    elif args.command == "sanitizer-build":
        payload = engine.call_tool(
            "sanitizer_build",
            {
                "target": args.target,
    ...
    elif args.command == "sanitizer-sweep":
        payload = engine.call_tool(
            "sanitizer_sweep",
            {
                "target": args.target,
                ...
                "per_input_timeout": args.per_input_timeout,
                "max_seconds": args.max_seconds,
                "runs_per_input": args.runs_per_input,
                "top": args.top,
                "declared_env": _parse_env_pairs(args.declared_env),
                "workspace_root": args.workspace_root,
            },
    ...
    elif args.command == "candidate-scoring":
        payload = engine.call_tool(
            "candidate_scoring",
            {
                "action": args.action,
                ...
                "votes": _load_json_arg(args.votes_json, None, default=None),
                "stream": args.stream,
                "rationale": args.rationale,
                "revision": args.revision,
                "labels": _load_json_arg(args.labels_json, None, default=None),
                "workspace_root": args.workspace_root,
            },
        )
    elif args.command == "campaign-db":
        payload = engine.call_tool(
            "campaign_db",
            {
                "action": args.action,
        ...
        payload = engine.call_tool(
            "schedule_plan",
            {"action": args.action, "now": args.now, "workspace_root": args.workspace_root},
        )
    elif args.command == "campaign-context":
        payload = engine.call_tool(
            "campaign_context",
            {"action": args.action, "target": args.target, "workspace_root": args.workspace_root},
        )
...
def _default_audit_roots() -> tuple[str, ...]:
    ...
    if plugin_root is not None:
        ...
        return (
            str(vendor_root / "agentic_fuzz_engine"),
            str(vendor_root / "agentic_fuzz_full"),
            str(plugin_root),
        )
...
def _plugin_root() -> Path | None:
    """Return an installed plugin root when the launcher supplied one.

    A source checkout remains useful for development, but installed plugins must
    audit their vendored implementation rather than inventing a checkout path.
    """
    raw = os.environ.get("AGENTIC_FUZZ_PLUGIN_ROOT")
...
def _load_json_argv(value: str, *, key: str) -> list[str]:
    parsed = json.loads(value)
    if not isinstance(parsed, list) or not parsed or not all(
        isinstance(item, str) and item for item in parsed
    ):
        raise ValueError(f"{key} must decode to a non-empty argv array")
    return parsed


def _parse_env_pairs(values: list[str] | None) -> dict[str, str] | None:
    if not values:
        return None
    result: dict[str, str] = {}
    for value in values:
        ...
        if not key or key in result:
            raise ValueError("--env keys must be non-empty and unique")
        result[key] = item
    return result
