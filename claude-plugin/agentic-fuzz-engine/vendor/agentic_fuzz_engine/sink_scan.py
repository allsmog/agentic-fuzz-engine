...
from .fork_inventory import _atomic_jsonl, _validate_output
...
def run_sink_scan(
    *,
    ...
    try:
        ...
        return {"ok": False, "blockers": [str(exc)], "rows_written": 0}
    ...
    try:
        ...
        return {"ok": False, "blockers": [f"unable to write sink inventory: {exc}"], "rows_written": 0}
...
def merge_sink_jsonl(
    *,
    ...
    read: dict[str, Any] = {"files": 0, "rows": 0, "bad_lines": 0, "missing": [], "blockers": []}
    for item in inputs:
        ...
        try:
            if path.stat().st_size > MAX_MERGE_INPUT_BYTES:
                read["blockers"].append(f"merge input exceeds byte cap: {path}")
                continue
        except OSError as exc:
            read["blockers"].append(f"unable to stat merge input {path}: {exc}")
            continue
        read["files"] += 1
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            for line in handle:
                if read["rows"] >= MAX_MERGE_ROWS:
                    read["blockers"].append(f"merge inputs exceed {MAX_MERGE_ROWS} rows")
                    break
                line = line.strip()
                if not line:
                    continue
    ...
    if read["blockers"]:
        return {
            "ok": False, "mode": "sink-merge", "out": str(out_path),
            "inputs_read": read["files"], "rows_read": read["rows"],
            "rows_written": 0, "bad_lines": read["bad_lines"],
            "missing_inputs": read["missing"], "blockers": read["blockers"],
        }

    out = Path(out_path).expanduser()
    try:
        out = _validate_output(out, None)
        _atomic_jsonl(out, [merged[key] for key in sorted(merged)])
    except (OSError, ValueError) as exc:
        return {
            "ok": False, "mode": "sink-merge", "out": str(out),
            "inputs_read": read["files"], "rows_read": read["rows"],
            "rows_written": 0, "bad_lines": read["bad_lines"],
            "missing_inputs": read["missing"], "blockers": [str(exc)],
        }
