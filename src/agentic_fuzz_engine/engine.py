...
class AgenticFuzzEngine:
    ...
    def tool_specs(self) -> list[dict[str, Any]]:
        return [
            ...
            _tool(
                "fork_scan",
                "Inventory vendor-package and exact build-consumer evidence as bounded candidates; results do not prove input reachability.",
                {
                    "source_root": "string",
                    "out": "string",
                    "vendor_markers": "array",
                    "manifest_globs": "array",
                    "consumer_root": "string",
                    "dpkg_info_dir": "string",
            ...
            _tool(
                "entry_scan",
                "Scan C/C++ sources for bounded candidate input boundaries with explicit evidence and confidence labels; results are not proof of reachability.",
                {
                    "source_root": "string",
                    "out": "string",
                    "lib_prefixes": "array",
                    "service_base_suffixes": "array",
                    "max_files": "integer",
                    "workspace_root": "string",
                },
            ),
            ...
            _tool(
                "differential_run",
                "Compare bounded argv-only replay implementations over a corpus and report behavioral candidates; disagreement is evidence, not a vulnerability verdict.",
                {
                    "target": "string",
                    "commands": {
                        "type": "array",
                    ...
                    "max_seconds": "number",
                    "top": "integer",
                    "accept_exit_codes": {"type": "array", "items": {"type": "integer"}},
                    "reject_exit_codes": {"type": "array", "items": {"type": "integer"}},
                    "declared_env": {
                        "type": "object",
            ...
            _tool(
                "sanitizer_build",
                "Build one bounded declared sanitizer variant using the hardened target recipe and explicit environment controls.",
                {"target": "string", "sanitizer": "string", "timeout_seconds": "number", "declared_env": "object", "workspace_root": "string"},
            ),
            _tool(
                "sanitizer_sweep",
                "Replay a corpus through one sanitizer variant with bounded resources and report parsed crash candidates; a clean sweep is not proof of absence.",
                {
                    "target": "string",
                    "sanitizer": "string",
                    "corpus_dir": "string",
            ...
            _tool(
                "candidate_scoring",
                "Record exact multi-lens judgments, report deterministic advisory scores, or calibrate a floor from explicit labels; scores never dispatch work automatically.",
                {
                    "action": "string",
                    "target": "string",
            ...
            _tool(
                "campaign_db",
                "Synchronize, inspect, or run a named bounded report over the derived campaign index; arbitrary SQL is intentionally unavailable.",
                {"action": "string", "report": "string", "target": "string", "limit": "integer", "workspace_root": "string"},
            ),
            _tool(
                "schedule_plan",
                "Generate or list freshness-checked advisory campaign ranks; this tool never launches or reprioritizes jobs.",
                {"action": "string", "now": "number", "workspace_root": "string"},
            ),
            _tool(
                "campaign_context",
                "Generate or show bounded campaign context with untrusted workspace data visibly delimited and source freshness disclosed.",
                {"action": "string", "target": "string", "workspace_root": "string"},
            ),
            _tool(
                "fleet_jobs",
    ...
    def call_tool(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        handlers: dict[str, ToolHandler] = {
            ...
            "fork_scan": self._fork_scan,
            "entry_scan": self._entry_scan,
            "sink_scan": self._sink_scan,
            "sink_coverage": self._sink_coverage,
            "valgrind_sweep": self._valgrind_sweep,
            "differential_run": self._differential_run,
            "sanitizer_build": self._sanitizer_build,
            "sanitizer_sweep": self._sanitizer_sweep,
            ...
            "candidate_scoring": self._candidate_scoring,
            "campaign_db": self._campaign_db,
    ...
    def _fork_scan(self, args: dict[str, Any]) -> dict[str, Any]:
        from .fork_inventory import run_fork_scan

        return run_fork_scan(
            source_root=args.get("source_root") or None,
            out_path=args.get("out") or None,
            workspace_root=args.get("workspace_root") or None,
            vendor_markers=_optional_string_list(args.get("vendor_markers"), key="vendor_markers"),
            manifest_globs=_optional_string_list(args.get("manifest_globs"), key="manifest_globs"),
            consumer_root=args.get("consumer_root") or None,
            dpkg_info_dir=args.get("dpkg_info_dir") or None,
            repository_alias=args.get("repository_alias") or None,
            env=dict(os.environ),
        )

    def _entry_scan(self, args: dict[str, Any]) -> dict[str, Any]:
        from .entry_scan import run_entry_scan

        return run_entry_scan(
            source_root=args.get("source_root") or None,
            out_path=args.get("out") or None,
            workspace_root=args.get("workspace_root") or None,
            lib_prefixes=_optional_string_list(args.get("lib_prefixes"), key="lib_prefixes"),
            service_base_suffixes=_optional_string_list(
                args.get("service_base_suffixes"), key="service_base_suffixes"
            ),
    ...
    def _differential_run(self, args: dict[str, Any]) -> dict[str, Any]:
        from .differential import differential_run

        auto = _strict_bool(args.get("auto"), key="auto", default=False)
        commands = _argv_matrix_arg(args.get("commands"), key="commands", allow_empty=auto)
        labels = _optional_string_list(args.get("labels"), key="labels")
        return differential_run(
            target=_required(args, "target"),
            commands=commands,
            labels=labels,
            corpus_dir=args.get("corpus_dir") or None,
            compare=str(args.get("compare") or "behavior"),
            auto=auto,
            build_timeout_seconds=args.get("build_timeout_seconds", 600.0),
            max_inputs=args.get("max_inputs", 2_000),
            per_input_timeout=args.get("per_input_timeout", 10.0),
            max_seconds=args.get("max_seconds", 900.0),
            top=args.get("top", 50),
            accept_exit_codes=args.get("accept_exit_codes", [0]),
            reject_exit_codes=args.get("reject_exit_codes", [1]),
            workspace_root=args.get("workspace_root") or None,
            env=dict(os.environ),
            declared_env=_declared_tool_env(args),
        )

    def _sanitizer_build(self, args: dict[str, Any]) -> dict[str, Any]:
        from .sanitizer_variants import sanitizer_build

        return sanitizer_build(
            target=_required(args, "target"),
            sanitizer=_required(args, "sanitizer"),
            timeout_seconds=args.get("timeout_seconds", 1_200),
            workspace_root=args.get("workspace_root") or None,
            env=dict(os.environ),
            declared_env=_declared_tool_env(args),
        )

    def _sanitizer_sweep(self, args: dict[str, Any]) -> dict[str, Any]:
        from .sanitizer_variants import sanitizer_sweep

        return sanitizer_sweep(
            target=_required(args, "target"),
            sanitizer=_required(args, "sanitizer"),
            corpus_dir=args.get("corpus_dir") or None,
            binary=args.get("binary") or None,
            max_inputs=args.get("max_inputs", 2_000),
            per_input_timeout=args.get("per_input_timeout", 15.0),
            max_seconds=args.get("max_seconds", 900.0),
            runs_per_input=args.get("runs_per_input", 1),
            top=args.get("top", 50),
            workspace_root=args.get("workspace_root") or None,
            env=dict(os.environ),
            declared_env=_declared_tool_env(args),
    ...
    def _candidate_scoring(self, args: dict[str, Any]) -> dict[str, Any]:
        from .candidate_scoring import calibrate, record_score, scoring_report
        from .workspace import resolve_workspace_root

        action = str(args.get("action") or "report")
        workspace_root = args.get("workspace_root") or None
        if action == "report":
            return scoring_report(workspace_root=workspace_root, env=dict(os.environ))
        if action == "record":
            votes = args.get("votes")
            if not isinstance(votes, list):
                raise ValueError("candidate_scoring votes must be an array")
            return record_score(
                resolve_workspace_root(workspace_root, env=dict(os.environ)),
                target=_required(args, "target"),
                hypothesis_id=_required(args, "hypothesis_id"),
                votes=votes,
                stream=str(args.get("stream") or "default"),
        ...
        if action == "calibrate":
            ...
            if not isinstance(labels, list):
                raise ValueError("candidate_scoring labels must be an array")
            return calibrate(labels=labels)
        raise ValueError("candidate_scoring action must be report, record, or calibrate")

    def _campaign_db(self, args: dict[str, Any]) -> dict[str, Any]:
        from .campaign_db import db_report, db_status, db_sync
        from .workspace import resolve_workspace_root

        action = str(args.get("action") or "status")
        workspace_root = args.get("workspace_root") or None
        if action == "sync":
            return db_sync(workspace_root=workspace_root, env=dict(os.environ))
        if action == "status":
            return db_status(resolve_workspace_root(workspace_root, env=dict(os.environ)))
        if action == "report":
            return db_report(
                name=_required(args, "report"),
                target=args.get("target") or None,
                limit=args.get("limit", 200),
                workspace_root=workspace_root,
                env=dict(os.environ),
            )
        raise ValueError("campaign_db action must be sync, status, or report")

    def _schedule_plan(self, args: dict[str, Any]) -> dict[str, Any]:
        from .scheduler import schedule_list, schedule_sync
        ...
        if action == "sync":
            return schedule_sync(**kwargs)
        if action == "list":
            return schedule_list(**kwargs)
        raise ValueError("schedule_plan action must be sync or list")

    def _campaign_context(self, args: dict[str, Any]) -> dict[str, Any]:
        from .primer import context_show, context_sync
        ...
        raise ValueError("campaign_context action must be sync or show")
    ...
    def _full_runtime_parity_audit(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if plugin_root is None:
            plugin_root = Path(__file__).resolve().parents[2] / "claude-plugin" / "agentic-fuzz-engine"
        payload = build_full_runtime_parity_audit(
            tool_names={tool["name"] for tool in self.tool_specs()},
            plugin_root=plugin_root,
            reference_root=self.reference_root,
        )
        payload["module_paths"] = {
            "agentic_fuzz_engine": str(Path(__file__).resolve()),
            "agentic_fuzz_full": str(Path(build_full_runtime_parity_audit.__code__.co_filename).resolve()),
        }
        return payload
...
def _optional_string_list(value: Any, *, key: str) -> list[str] | None:
    if value is None:
        return None
    return _string_list(value, key=key)


def _argv_matrix_arg(value: Any, *, key: str, allow_empty: bool = False) -> list[list[str]]:
    if value in (None, []):
        if allow_empty:
            return []
        raise ValueError(f"{key} must be a non-empty array of argv arrays")
    if not isinstance(value, list) or not all(
        ...
        raise ValueError(f"{key} must be an array of non-empty argv arrays")
...
def _plugin_root() -> Path | None:
    """Return the installed plugin root, if this is a plugin launch."""
