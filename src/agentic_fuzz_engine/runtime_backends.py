...
def _development_repo_root() -> Path | None:
    """Return an explicitly configured checkout, never an inferred plugin path."""
    explicit = os.environ.get("AGENTIC_FUZZ_ENGINE_ROOT")
    if explicit and (Path(explicit).expanduser() / "tools" / "bin").is_dir():
        return Path(explicit).expanduser().resolve()
    if os.environ.get("AGENTIC_FUZZ_PLUGIN_ROOT"):
        return None
...
def _docker_wrapper_image_check(
    ...
    *,
    ...
    if repo_root is None:
        return None
    repo_tools = repo_root / "tools" / "bin"
    try:
