...
def _generate_type_enum(spec: dict[str, Any], *, target_dir: Path, placeholders: Mapping[str, str]) -> dict[str, Any]:
    ...
    (target_dir / "harness.cpp").write_text(harness, encoding="utf-8")
...
def _generate_symbolic_string(
    ...
    for sink in sinks:
        ...
        written.append(str(harness_path))
        targets.append(
            {
                "name": slug,
...
def _render_prototype(extraction: dict[str, Any]) -> str:
    params = ", ".join(
        f"{param['type']} {param['name']}".strip() for param in extraction["params"]
    )
    declaration = f"{extraction['returns']} {extraction['name']}({params});"
