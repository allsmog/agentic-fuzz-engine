...
class ContainerBuildTests(unittest.TestCase):
    def test_total_timeout_is_shared_across_real_build_steps(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            sleep = [sys.executable, "-c", "import time; time.sleep(0.09)"]
            config = {"steps": [
                {"name": "first", "argv": sleep, "env": {}},
                {"name": "second", "argv": sleep, "env": {}},
            ]}

            started = time.monotonic()
            result = build_target(
                project="localfuzz/c/demo", workspace_root=ws, config_override=config,
                timeout_seconds=1, total_timeout_seconds=0.12, env={},
            )
            elapsed = time.monotonic() - started

        self.assertFalse(result["ok"], result)
        self.assertTrue(any("timeout" in blocker or "budget exhausted" in blocker for blocker in result["blockers"]))
...
class CampaignRoundsTests(unittest.TestCase):
    ...
    def test_missing_klee_extraction_never_imports_the_current_directory(self) -> None:
        from agentic_fuzz_engine.campaign_rounds import run_campaign_rounds

        class MissingExtractionEngine(_StubEngine):
            def call_tool(self, name: str, args: dict) -> dict:
                if name == "symbolic_worker_run":
                    self.calls.append((name, args))
                    return {
                        "ok": False,
                        "output_dir": str(Path(tmp) / "missing-output"),
                        "result": {},
                        "blockers": ["KLEE did not run"],
                    }
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            crash_dir.mkdir()
            (crash_dir / "crash-1").write_bytes(b"boom")
            host_file = Path(tmp) / "not-a-klee-seed"
            host_file.write_bytes(b"HOST_DATA")
            engine = MissingExtractionEngine(crash_dir)

            result = run_campaign_rounds(
                engine, project="demo", rounds=1, fuzz_seconds=1,
                klee_config="demo.ci.json", klee_every=1,
                workspace_root=ws, env={}, min_free_gb=0,
            )

            klee = result["rounds"][0]["klee"]
            self.assertEqual(klee["seeds_merged"], 0)
            self.assertFalse(klee["ok"])
            self.assertEqual(
                list((ws / "work" / "demo" / "seeds").iterdir()), []
            )
            self.assertEqual(host_file.read_bytes(), b"HOST_DATA")

    def test_seed_merge_rejects_relative_source_symlinks_and_destination_symlinks(self) -> None:
        from agentic_fuzz_engine.campaign_rounds import _merge_seeds
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "absolute paths"):
                _merge_seeds(Path(""), corpus, prefix="klee")

            linked = source / "linked"
            linked.symlink_to(victim)
            with self.assertRaisesRegex(ValueError, "non-regular entry"):
                _merge_seeds(source, corpus, prefix="klee")
            linked.unlink()

            payload = b"safe-seed"
            (source / "seed").write_bytes(payload)
            digest = sha256(payload).hexdigest()[:20]
            (corpus / f"klee-{digest}").symlink_to(victim)
            with self.assertRaisesRegex(ValueError, "unsafe or mismatched"):
                _merge_seeds(source, corpus, prefix="klee")
            self.assertEqual(victim.read_bytes(), b"KEEP")

    def test_seed_merge_bounds_directory_enumeration_before_copying(self) -> None:
        from agentic_fuzz_engine import campaign_rounds
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(campaign_rounds, "MAX_KLEE_SEED_DIRECTORY_ENTRIES", 1):
                with self.assertRaisesRegex(ValueError, "exceeds 1 entries"):
                    campaign_rounds._merge_seeds(source, corpus, prefix="klee")
            self.assertEqual(list(corpus.iterdir()), [])

    def test_seed_merge_rejects_source_and_corpus_directory_preopen_swaps(self) -> None:
        from agentic_fuzz_engine import campaign_rounds

        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp).resolve()
            source = root / "source"
            source_saved = root / "source-saved"
            attacker = root / "attacker"
            corpus = root / "corpus"
            source.mkdir()
            attacker.mkdir()
            corpus.mkdir()
            (source / "seed").write_bytes(b"SAFE")
            (attacker / "seed").write_bytes(b"EVIL")
            real_open = campaign_rounds._open_absolute_directory

            def swap_source_then_open(path):
                if path == source:
                    source.rename(source_saved)
                    attacker.rename(source)
                    try:
                        return real_open(path)
                    finally:
                        source.rename(attacker)
                        source_saved.rename(source)
                return real_open(path)

            with mock.patch.object(
                campaign_rounds, "_open_absolute_directory", side_effect=swap_source_then_open
            ):
                with self.assertRaisesRegex(ValueError, "changed before opening"):
                    campaign_rounds._merge_seeds(source, corpus, prefix="klee")
            self.assertEqual(list(corpus.iterdir()), [])

            corpus_saved = root / "corpus-saved"
            victim = root / "victim-corpus"
            victim.mkdir()

            def swap_corpus_then_open(path):
                if path == corpus:
                    corpus.rename(corpus_saved)
                    victim.rename(corpus)
                    try:
                        return real_open(path)
                    finally:
                        corpus.rename(victim)
                        corpus_saved.rename(corpus)
                return real_open(path)

            with mock.patch.object(
                campaign_rounds, "_open_absolute_directory", side_effect=swap_corpus_then_open
            ):
                with self.assertRaisesRegex(ValueError, "changed before opening"):
                    campaign_rounds._merge_seeds(source, corpus, prefix="klee")
            self.assertEqual(list(corpus.iterdir()), [])
